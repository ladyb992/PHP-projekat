<?php
	...
	Flight::register('db', 'Database', array('biblioteka'));
	...
	Flight::route('GET /rez.json', function() {
		header("Content-Type: application/json; charset=utf-8");
		$db = Flight::db();
		...
		if(!isset($_GET['search'])) {
			$db->select2(" rezervacija ", ' * ', "knjiga", "knjigaID", "knjigaID", "korisnici", "korisnik","username", null, null);
			$niz = array();
			while($red = $db->getResult()->fetch_object()) {
				$niz[] = $red;
			}
...
Flight::route('GET /rezervacija.json', function() {
		...
		$podaci = json_decode($podaci_json);
			...
			$db->select(" rezervacija ", ' * ', "knjiga", "knjigaID", "knjigaID", null, null);
			$niz = array();
			while($red = $db->getResult()->fetch_object()) {
				$niz[] = $red;
			}
	...
	//vrati knjige odredjenog pisca
		...
		Flight::route('GET /rezervacija/@username.json', function($username) {
		header("Content-Type: application/json; charset=utf-8");
		$db = Flight::db();
		...
		if(!isset($_GET['search'])) {
			$db->select(" rezervacija ", ' * ', "korisnici", "korisnik", "username", "rezervacija.korisnik = '". $username."'", null);
			$niz = array();
			while($red = $db->getResult()->fetch_object()) {
				$niz[] = $red;
			}
...
Flight::route('GET /korisnik.json', function() {
	...
	Flight::route('POST /rezervacija', function() {
		...
		return false;
			...
			if(!property_exists($podaci,'knjigaID') || !property_exists($podaci,'korisnik') || !property_exists($podaci,'datum') ) {
				...
				if($db->insert("rezervacija","knjigaID,korisnik,datum",array($podaci_query['knjigaID'], $podaci_query['korisnik'],$podaci_query['datum']))) {
	...
	Flight::route('DELETE /rezervacija/@rezervacijaID', function($rezervacijaID) {
		header("Content-Type: application/json; charset=utf-8");
		$db = Flight::db();

		if($db->delete("rezervacija", array("rezervacijaID"),array($rezervacijaID))) {
			$odgovor["poruka"] = "Rezervacije je otkazana!";
			$json_odgovor = json_encode($odgovor,JSON_UNESCAPED_UNICODE);
			echo $json_odgovor;
			return false;
		}
		else {
			$odgovor["poruka"] = "Došlo je do greške pri pokušaju otkazivanja rezervacija!";
			$json_odgovor = json_encode($odgovor,JSON_UNESCAPED_UNICODE);
			echo $json_odgovor;
			return false;
		}
